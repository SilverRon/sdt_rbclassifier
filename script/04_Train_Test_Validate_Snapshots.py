...
def main(args):
    ...
    # 3. Output Management
    output_manager = OutputManager(pipeline='snapshot', run_name=args.version)
    output_manager.write_args(vars(args))
    output_manager.write_features(['sci', 'sub']) # Standard features for snapshot models
    checkpoint_callback = ModelCheckpoint(
        monitor='val_loss',
        dirpath=str(output_manager.get_path('checkpoints')),
    ...
    # 4. Logger
    logger = TensorBoardLogger(str(output_manager.get_path('logs')), name=args.version)
    ...
    if not args.fast_dev_run:
        ...
        visualize_training_results(trainer, model, dm, args, output_manager, final_metrics=metrics)
    else:
